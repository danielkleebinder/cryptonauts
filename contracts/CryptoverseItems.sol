...
contract CryptoverseItems is Cryptoverse {
    ...
    event ItemTransferred(address previousOwner, address newOwner, uint itemId);
    ...
    function buyItem(uint _itemTypeId) external {
        require(isItemType(_itemTypeId), "This is not an item you can buy");

        Item memory itemType = itemTypes[_itemTypeId];

        // Burn the items cost
        require(balanceOf(msg.sender) >= itemType.cost, "This item is too expensive for you");
        burn(itemType.cost);
    ...
    function levelUpItem(uint _itemId) external onlyItemOwnerOf(_itemId) {
        ...
        require(balanceOf(msg.sender) >= item.cost, "This upgrade is too expensive for you");
        require(item.level < maxItemLevel, "You cannot further upgrade this item");
        burn(item.cost);
    ...
    /**
     * @dev Transfers a certain item from one player to another one. Only the item owner
     *      can perform this action.
     * @param _newOwner The new owners address.
     * @param _itemId   The item which should be transferred.
     */
    function transferItemTo(address _newOwner, uint _itemId) external onlyItemOwnerOf(_itemId) {
        ...
        emit ItemTransferred(msg.sender, _newOwner, _itemId);
    }
